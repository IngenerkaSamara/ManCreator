...
int main()
{
    ...
    buttons[1] = {90,0, "Ëþäè","people" };
    buttons[2] = {180,0, "Æèâîòíûå","Animals"};
    buttons[3] = {270,0, "Ïàìÿòíèêè", "Pamatnik"};
    buttons[4] = {360,0, "Çäàíèÿ", "building"};
    buttons[5] = {430,0, "On",""};
    buttons[6] = {500,0, "Off",""};
    buttons[7] = {570,0, "?",""};
    ...
    while (!GetAsyncKeyState(VK_ESCAPE))
    {
        ...
        txTextOut(1000,700, "Âûáåðèòå ÷å-íèòü");
        ...
        else if (buttons[7].Click())
        {
            ...
            while (stop == false)
            {
                ...
                txDrawText(0, 0,1200,800,
                    ...
                    "     ( ×òîá âûáðàòü ïðåäìåò íàæìè íà íåãî ëåâîé êíîïêîé ìûøè )\n"
                    " \n    1 - ó íàñ åñòü óäàëåíèå , âûáåðè ïðåäìåò êîòîðûé õî÷åøü ÓÍÈ×ÒÎÆÈÒÜ è íàæìè Delete"
                    " \n    2 - íà On è Off òû ìîæåøü âêëþ÷àòü è âûêëþ÷àòü ìóçûêó"
                    " \n    3 - âûáåðè îò äîìà äî çäàíèÿ è ïåðåíåñè îáúåêò íà ýêðàí"
                    " \n    4 - íàæàâ íà êíîïêó R(Ê) òû ìîæåøü îòçåðêàëèòü âûáðàííûé ïðåäìåò\n"
                    " \n"
        ...
        //Âûáðàííàÿ êàðòèíêà äâèæåòñÿ çà ìûøêîé
        ...
        //ïðè îòïóñêàíèè ìûøêè ñáðàñûâàåòñÿ âûáîð êàðòèíêè
        if (!(txMouseButtons() & 1) && pictures[nomer_kartinki].clicked)
        {
